...
def evaluate_gatekeepers(p0_params):
    """
    Step 1: The Gatekeeper Function. 
    Acts as a binary switch. Checks critical thresholds.
    """
    if p0_params.get("soc", 1.0) < 0.15: return 0
    if p0_params.get("thermal_margin", 1.0) < 0.05: return 0
    if p0_params.get("reaction_wheel_rpm", 0.0) >= 0.95: return 0
    if p0_params.get("sensor_calibrated", 1.0) < 1.0: return 0
    if p0_params.get("conjunction_prob", 0.0) >= 0.01: return 0
    if p0_params.get("is_task_locked", 0.0) >= 1.0: return 0
    
    return 1

def calculate_base_capability(state_vector, weight_vector):
    """
    Step 2: The Weighted Sum Engine. 
    Dot product of parameters and their weights.
    """
    c_raw = 0.0
    for param, val in state_vector.items():
